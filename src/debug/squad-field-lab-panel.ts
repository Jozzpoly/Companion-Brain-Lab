          ...
          ${state.trialComparison
            ? state.trialComparison.members.map((member) => {
                ...
                return (
                  ...
                  `<div class="squad-field-debug-subline">${memberLabel(member.memberId)} exposure · first/last blocked A ${eventTick(a?.firstBlockedTickOffset)}/${eventTick(a?.lastBlockedTickOffset)} → B ${eventTick(b?.firstBlockedTickOffset)}/${eventTick(b?.lastBlockedTickOffset)} · first contact A ${eventTick(a?.firstContactTickOffset)} → B ${eventTick(b?.firstContactTickOffset)} · block episodes A ${a?.blockedEpisodes ?? 0} → B ${b?.blockedEpisodes ?? 0} · final A ${a?.finalStatus ?? "n/a"} / ${targetError(a?.finalTargetError)} → B ${b?.finalStatus ?? "n/a"} / ${targetError(b?.finalTargetError)}</div>`
                );
              }).join("")
