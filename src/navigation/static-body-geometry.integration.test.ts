...
describe("foundation static placement parity with Rapier occupancy", () => {
  it("agrees that exact obstacle tangency is not a free placement", async () => {
    const physical = await RapierPhysicalWorld.create(fixture);
    try {
      const point = { x: 5.2, y: 2 };
      expect(physical.staticCircleOccupancy(point, 0.3).clear).toBe(false);
      expect(helper(point)).toBe(false);
    } finally {
      physical.dispose();
    }
  });
