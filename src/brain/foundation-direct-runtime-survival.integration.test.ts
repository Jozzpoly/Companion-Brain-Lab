...
describe("foundation DIRECT full-chain runtime survival", () => {
  it("survives hard-egress pressure and resumes normal motion across 180 World steps", async () => {
    const fixture: ScenarioSpec = {
      id: "pillar",
      label: "direct dynamic runtime survival",
      width: 12,
      height: 8,
      actors: [
    ...
    try {
      for (let step = 0; step < 180; step += 1) {
        ...
        if (!companion) throw new Error("DIRECT survival fixture missing companion");
        ...
        const repair = brain.debugState().repair;
        ...
        if (!postCompanion) throw new Error("DIRECT survival fixture missing post-step companion");
        ...
        const progress = brain.observeOutcome({
          snapshot: post,
          objectiveKey: "foundation:direct-dynamic-survival",
          target,
          routePlan: postRoute
        });
