...
const ARTIFACT_DIR = "artifacts/os-prep3-ph05-doorway-reversal";
...
async function singleStep(page,expected){
  invariant(await currentTick(page)===expected,`PH-05 tick drift before t${expected}.`);
  await page.locator('[data-action="single-step"]').click();
  await waitForPanel(page,text=>panelTick(text)===expected+1,15000,`PH-05 t${expected}->${expected+1}`);
}
...
async function captureIncident(page,pathOut){
  ...
  invariant(p,"PH-05 incident download produced no path.");
...
function compareTwins(aFrames,bFrames){
  invariant(aFrames.length===TOTAL_TICKS&&bFrames.length===TOTAL_TICKS,"PH-05 Twin frame count mismatch.");
  let maxPositionError=0,maxVelocityError=0;
  for(let i=0;i<TOTAL_TICKS;i++){
    const a=physicalView(aFrames[i]),b=physicalView(bFrames[i]);
    invariant(a.observationTick===b.observationTick,`PH-05 observation mismatch @${i}`);
    invariant(a.outcomeTick===b.outcomeTick,`PH-05 outcome mismatch @${i}`);
    invariant(a.playerControlMove.x===b.playerControlMove.x&&a.playerControlMove.y===b.playerControlMove.y,`PH-05 control mismatch @t${a.observationTick}`);
    ...
    invariant(JSON.stringify(a.contacts)===JSON.stringify(b.contacts),`PH-05 contacts differ @t${a.outcomeTick}`);
    invariant(a.postState===b.postState,`PH-05 post-state mismatch @t${a.outcomeTick}`);
    invariant(a.routeStatus===b.routeStatus&&a.routePath===b.routePath,`PH-05 route mismatch @t${a.observationTick}`);
  }
  invariant(maxPositionError<=1e-9,`PH-05 research lens changed position: ${maxPositionError}`);
  invariant(maxVelocityError<=1e-9,`PH-05 research lens changed velocity: ${maxVelocityError}`);
  return {maxPositionError,maxVelocityError};
}
function beatForTick(tick){ return tick<120?"outbound-plus-x":"immediate-return-minus-x"; }
...
function summarize(frames){
  ...
  const initial=frames[0].observation.playerPosition;
  let outboundCrossTick=null, returnCrossTick=null;
  let routePathChanges=0, previousRoutePath=frames[0].decision.routePath;
  const routeChangeTicks=[];
  for(const frame of frames){
    const t=frame.observation.worldTick;
    const x=frame.observation.playerPosition.x;
    if(outboundCrossTick===null && t<120 && x>=6.4) outboundCrossTick=t;
    if(t>=120 && outboundCrossTick!==null && returnCrossTick===null && x<=5.6) returnCrossTick=t;
    if(frame.decision.routePath!==previousRoutePath){
      routePathChanges+=1;
      routeChangeTicks.push({tick:t,from:previousRoutePath,to:frame.decision.routePath});
  ...
  const closest=frames.map(f=>({tick:f.observation.worldTick,value:distance(f.observation.playerPosition,f.observation.companionPosition)}))
    .reduce((a,b)=>b.value<a.value?b:a);
  const outbound=steps.filter(s=>s.beat==="outbound-plus-x");
  const inbound=steps.filter(s=>s.beat==="immediate-return-minus-x");
  const sum=v=>v.reduce((a,b)=>a+b,0);
  return {
    initialPlayerPosition:initial,
    finalObservedPlayerPosition:frames.at(-1).observation.playerPosition,
    requestedProgress,
    actualProgress,
    progressDeficit:requestedProgress-actualProgress,
    progressFraction:actualProgress/requestedProgress,
    outbound:{
      requestedProgress:sum(outbound.map(s=>s.expected)),
      actualProgress:sum(outbound.map(s=>s.forward)),
      deficit:sum(outbound.map(s=>s.deficit)),
      doorwayCrossTick:outboundCrossTick
    },
    return:{
      requestedProgress:sum(inbound.map(s=>s.expected)),
      actualProgress:sum(inbound.map(s=>s.forward)),
      deficit:sum(inbound.map(s=>s.deficit)),
      doorwayReturnTick:returnCrossTick
    },
...
async function runTwin({browser,research}){
  ...
  await waitForPanel(page,t=>t.includes("PAUSED"),10000,"PH-05 pause");
  await page.locator('[data-action="scenario-doorway"]').click();
  await waitForPanel(page,t=>t.includes("scenario Narrow doorway")&&t.includes("PAUSED")&&t.includes("mode SPATIAL")&&t.includes("actuator NATURAL")&&t.includes("A1 OFF")&&panelTick(t)===0,15000,"PH-05 doorway reset");
  if(research){
    await page.locator('[data-action="cycle-a1-authority"]').click();
    await waitForPanel(page,t=>t.includes("A1 DIRECT")&&t.includes("PAUSED"),10000,"PH-05 A1 pass-through");
  }
  ...
  await setKeys(page,[]);
  invariant(tick===TOTAL_TICKS,"PH-05 story tick mismatch.");
  const incident=await captureIncident(page,`${dir}/incident.json`);
  invariant(incident.schema==="companion-brain-lab-owner-sandbox-incident-v1","PH-05 incident schema mismatch.");
  invariant(incident.capture.scenario==="doorway","PH-05 scenario mismatch.");
  invariant(incident.capture.mode==="spatial"&&incident.capture.actuator==="natural","PH-05 baseline mode/actuator mismatch.");
  invariant(incident.capture.a1Variant===(research?"direct":"off"),"PH-05 A1 variant mismatch.");
  invariant(incident.capture.tick===TOTAL_TICKS&&incident.frames.length===TOTAL_TICKS,"PH-05 incident window mismatch.");
  if(process.env.GITHUB_SHA){
    invariant(incident.build.sourceSha===process.env.GITHUB_SHA,"PH-05 source SHA mismatch.");
    invariant(incident.build.state==="PINNED_SOURCE_SHA","PH-05 build not pinned.");
  }
  let a11f=null;
  if(research){
    a11f=await page.evaluate(()=>window.__authorityA11fBrowserBridge?.snapshot()??null);
    invariant(a11f?.authority==="PASS_THROUGH_ONLY","PH-05 A1 authority changed.");
    invariant(a11f.lastBridgeError===null,"PH-05 A1 bridge error.");
    invariant(a11f.frameCount===TOTAL_TICKS,`PH-05 expected ${TOTAL_TICKS} A1 frames.`);
    invariant(a11f.frames.every(f=>JSON.stringify(f.baselineCompanionIntent)===JSON.stringify(f.selectedCompanionIntent)),"PH-05 A1 lens changed live intent.");
    await writeFile(`${dir}/a1-1f.json`,JSON.stringify(a11f,null,2));
  }
  invariant(await page.locator("#runtime-fault-sentinel").count()===0,"PH-05 runtime fault sentinel visible.");
  invariant(errors.page.length===0&&errors.console.length===0&&errors.requests.length===0,`PH-05 browser errors: ${JSON.stringify(errors)}`);
...
try{
  ...
  const summary={
    schema:"companion-brain-lab-os-prep3-ph05-doorway-reversal-v1",
