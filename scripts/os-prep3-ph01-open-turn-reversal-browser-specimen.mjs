...
const ARTIFACT_DIR = "artifacts/os-prep3-ph01-open-turn-reversal";
...
const STORY = [
  ...
  { id: "turn-plus-y", keys: ["s"], steps: 36 },
  { id: "brief-stop", keys: [], steps: 18 },
  { id: "reverse-minus-y", keys: ["w"], steps: 36 },
...
try {
  ...
  const summary = {
    schema: "companion-brain-lab-os-prep3-ph01-open-turn-reversal-v1",
  ...
  console.log(`[OS_PREP3_PH01_OPEN_TURN_REVERSAL] ${JSON.stringify(summary)}`);
} finally {
  await browser?.close();
